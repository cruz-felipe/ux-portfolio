  ...
  return (
    ...
    <main id="main-content" role="main" className="page-enter" style={{ background: "var(--paper)", color: "var(--ink)", minHeight: "100vh" }}>
      ...
      <section className="case-hero-inner" style={{
        padding: "var(--space-xl) var(--col-pad) var(--space-lg)",
        opacity: visible ? 1 : 0,
        ...
        <div className="case-callout" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-xl)", paddingTop: "1.5rem", borderTop: "0.5px solid var(--border)" }}>
          <div>
      ...
      {data.metrics.length > 0 && (
        <section className="metrics-section" style={{ borderTop: "0.5px solid var(--border)", borderBottom: "0.5px solid var(--border)", padding: "var(--space-lg) var(--col-pad)" }}>
      ...
      {/* Context */}
      <section className="context-section" style={{ padding: "var(--space-lg) var(--col-pad)", borderBottom: "0.5px solid var(--border)" }}>
      ...
      <div ref={sectionsRef}>
        {data.sections.map((section, si) => (
          <div key={si}>
            <section style={{
              padding: "var(--space-lg) var(--col-pad)", borderBottom: "0.5px solid var(--border)",
              display: "grid", gridTemplateColumns: "var(--sidebar) 1fr", gap: "5rem", alignItems: "start",
            }} className="case-section">
              <h2 style={{ fontFamily: "var(--font-body)", fontSize: "10px", fontWeight: 500, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", margin: 0, paddingTop: "5px", lineHeight: 1.6 }}>
                {section.title}
              </h2>
              <div className="case-section-body" style={{ maxWidth: "var(--content)", width: "100%" }}>
                {section.pullquote && (
                ...
                {Array.isArray(section.body) ? section.body.map((para, pi) => (
                  <p key={pi} className={pi === 0 && !section.pullquote ? "case-body-lead" : undefined} style={{ fontFamily: "var(--font-body)", fontSize: pi === 0 && !section.pullquote ? undefined : "15px", fontWeight: 400, lineHeight: pi === 0 && !section.pullquote ? undefined : 1.8, color: "var(--ink)", opacity: pi === 0 && !section.pullquote ? undefined : 0.78, marginBottom: pi < (section.body as string[]).length - 1 ? "1.5rem" : 0 }}>{para}</p>
                )) : (
            ...
            {section.screens && section.screens.length > 0 && (
              <div className="screen-gallery-section" style={{ padding: "var(--space-lg) var(--col-pad)", borderBottom: "0.5px solid var(--border)" }}>
                <ScreenGallery screens={section.screens} narrow={section.narrowScreens} />
              </div>
            )}

            {data.artifacts[si]?.component && (
              <section className="artifact-section" style={{ padding: "var(--space-lg) var(--col-pad)", borderBottom: "0.5px solid var(--border)", background: "color-mix(in srgb, var(--ink) 3.5%, var(--paper))" }}>
      ...
      {/* More work */}
      <section aria-label="More work" style={{ padding: "var(--space-lg) var(--col-pad)", borderTop: "0.5px solid var(--border)", display: "grid", gridTemplateColumns: "var(--sidebar) 1fr", gap: "5rem", alignItems: "start" }} className="case-section">
